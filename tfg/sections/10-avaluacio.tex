...
\begin{table}[H]
    ...
    \begin{tabular}{@{}cl>{\raggedright\arraybackslash}p{4.5cm}l@{}}
        ...
        O8 & Agents en segon pla (3 agents) & embedding-backfill només (D5, treball futur) & 🟡 parcial \\
        O9 & Knowledge graph com a primitiva exploratòria & \texttt{/graph} amb 2D/3D + 3 tipus arestes & ✅ \\
        \bottomrule
    \end{tabular}
\end{table}

\paragraph{Discussió.} 8 de 9 objectius assolits íntegrament. L'objectiu O8 (agents) està parcial: la decisió D5 documentada al registre de decisions discuteix els tres agents previstos (\texttt{embedding-backfill}, \texttt{auto-tag}, \texttt{weekly-digest}); el primer és el més simple (no involucra LLM downstream) i és el que es prioritza per al lliurament. Els altres dos queden documentats al disseny (\S\ref{sec:disseny-agents}) com a treball futur amb diagrames de seqüència complets.

L'argumentació per a aquesta priorització està al capítol~\ref{ch:valoracio} (\emph{Què faria diferent}): construir tres agents amb capa LLM downstream, cadascun amb la seva pròpia anàlisi Lethal Trifecta i el seu propi D3, hauria consumit el temps de polit del memoir. La decisió de \emph{document-en-profunditat + implementar-el-més-simple} és coherent amb l'esperit del TFG: demostrar el patró amb una instància representativa, no construir tres variants del mateix.
