...
\begin{itemize}
    \item \textbf{Tag suggestions} (\S\ref{sec:tag-suggestion}): el model proposa etiquetes per a una nota. Aquesta proposta pot reflectir biaixos lèxics (preferir certes formes d'etiquetatge sobre altres). Mitigació: l'usuari decideix sempre quina proposta accepta o rebutja, i pot crear etiquetes manualment sense passar pel model.
...
\begin{itemize}
    ...
    \item \textbf{No discriminació per idioma del contingut}: el filtre D3 (\S\ref{sec:red-team-promptfoo}) inclou variants d'atac en català, espanyol, francès i italià explícitament. La validació empírica mostra detection rate del 100\% en català, demostrant que la robustesa lingüística no és anglèsofiloproblema.
\end{itemize}
