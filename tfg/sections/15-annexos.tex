...
Aquest annex resumeix el flux d'usuari principal de Synapse Notes des de l'autenticació fins a la consulta amb el xat RAG. Les captures de pantalla estan disponibles al repositori sota \texttt{docs/screenshots/} (no incloses al PDF per a mantenir-lo lleuger).
...
\chapter{Configuració del servidor MCP per a Claude Desktop}
\label{ch:annex-mcp}

Aquest annex descriu com configurar Claude Desktop perquè es connecti al servidor MCP remot de Synapse Notes. Aquesta configuració permet a l'usuari invocar les eines des de fora del navegador, dins una sessió de xat amb Claude.

\section*{Pas 1: Obtenir un JWT de Supabase}

El servidor MCP requereix autenticació via JWT de Supabase al header \texttt{Authorization: Bearer <token>}. El JWT es pot extreure de la sessió web autenticada:

\begin{enumerate}
    \item Autenticar-se a \url{https://synapse-notes.vercel.app}.
    \item Obrir DevTools del navegador (\texttt{F12}).
    \item Anar a la pestanya \emph{Application} (Chrome) o \emph{Storage} (Firefox).
    \item Sota \emph{Cookies}, buscar les entrades \texttt{sb-*-auth-token}. Pot ser-hi una sola o estar partida en \texttt{.0}, \texttt{.1}, etc.
    \item Concatenar els valors, fer base64-decode si comencen amb \texttt{base64-}, i parsejar com a JSON. El camp \texttt{access\_token} és el JWT.
\end{enumerate}

\section*{Pas 2: Configurar \texttt{claude\_desktop\_config.json}}

A Claude Desktop, editar el fitxer de configuració (la seva ubicació depèn del sistema operatiu; vegeu la documentació oficial d'Anthropic):
...
\textbf{Important}: el JWT té una durada limitada (1 hora per defecte). Cal renovar-lo refrescant la sessió web i actualitzant el fitxer de configuració.

\section*{Pas 3: Verificar la connexió}

A Claude Desktop, iniciar una nova conversa i preguntar:

\begin{quote}
``Quines eines tens disponibles del servidor synapse-notes?''
\end{quote}

Claude hauria de respondre amb la llista de les 8 eines exposades. Provar una crida senzilla:

\begin{quote}
``Cerca a les meves notes la paraula \emph{tutorial}.''
\end{quote}

Claude invocarà \texttt{search\_notes} via MCP, retornarà els resultats i els resumirà.
