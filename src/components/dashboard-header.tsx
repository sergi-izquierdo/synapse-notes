...
import { BrainCircuit, Sun, Moon, Settings, Network } from "lucide-react";
    ...
    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-card/80 backdrop-blur-md p-4 rounded-lg border border-border/60 sticky top-4 z-10 shadow-sm">
      <div className="flex items-center gap-3">
        <div className="bg-primary/10 p-2 rounded-lg">
          <BrainCircuit className="h-6 w-6 text-primary" aria-label={t.header.logoAlt} />
        </div>
        <div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Link
          href="/graph"
          className="inline-flex items-center justify-center rounded-md p-2 text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors"
          aria-label={t.header.openGraph}
          title={t.header.noteGraphTitle}
        >
